...
$id=intval($_POST["_id"]);
unset($_POST["_id"]);
if(!isset($_POST["reference"])||$_POST["reference"]=="기타")$_POST["reference"]=$_POST["reference_etc"];
unset($_POST["reference_etc"]);
$new=array();
...
$query="";
$log="INSERT INTO logs(user_id,text_id,attribute,old_value,new_value) VALUES";
if($id<=$tot)
{
	...
	foreach($new as $col=>$val)
	{
		...
		if($val!=$old[$col])$log.="(".$_SESSION["user_id"].",".$id.",'".$col."',".$old[$col].",".$val."),";
	}
...
else
{
	...
	{
		$query.=$val.",";
		if($val!="NULL")$log.="(".$_SESSION["user_id"].",".$id.",'".$col."',NULL,".$val."),";
	}
...
if(!$result)
{
	Header("Location: input.html?id=".$id."&error");
	exit();
}
mysqli_query($connection,$log);
Header("Location: input.html?id=".$id."&save_success");
?>